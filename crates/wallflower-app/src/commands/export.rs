...
/// Export a sample (section or loop) as a time-sliced audio file with JSON sidecar metadata.
/// Unlike export_audio, this accepts raw jam_id + time range parameters directly,
/// enabling export of sections and loops without creating a bookmark first (D-16, D-17).
#[command]
pub async fn export_sample_audio(
    ...
    let (jam, export_root, format_ext, bit_depth) = {
        let db = state.db.lock().map_err(|e| e.to_string())?;
        ...
        (jam, PathBuf::from(export_root), format_ext, bit_depth)
    };

    let jam_name = &jam.original_filename;
    let source_path = Path::new(&jam.file_path);

    // Resolve export path with collision avoidance (T-07-02)
    let dest_path = export::sanitize::resolve_export_path(
        &export_root,
        jam_name,
        &sample_name,
    ...
    // No exports table record -- no bookmark_id to associate (D-16)
...
/// Separate stems for a sample (section or loop) using the gRPC sidecar.
/// Unlike separate_stems, this accepts raw jam_id + time range parameters
/// directly, enabling stem separation without creating a bookmark first (D-16, D-17).
#[command]
pub async fn separate_sample_stems(
    app: AppHandle,
    state: tauri::State<'_, AppState>,
    jam_id: String,
    start_seconds: f64,
    ...
    let (jam, model_name, memory_limit_gb) = {
        let db = state.db.lock().map_err(|e| e.to_string())?;
        let jam = wallflower_core::db::get_jam(&db.conn, &jam_id)
            .map_err(|e| e.to_string())?
            .ok_or_else(|| format!("Jam not found: {}", jam_id))?;
        ...
        let model_name = wallflower_core::db::get_setting(&db.conn, "separation_model")
            ...
            .flatten()
            .unwrap_or_else(|| "htdemucs".to_string());
        let memory_limit_gb: f64 =
            wallflower_core::db::get_setting(&db.conn, "separation_memory_limit_gb")
                .ok()
                .flatten()
                .and_then(|v| v.parse().ok())
                .unwrap_or(4.0);

        (jam, model_name, memory_limit_gb)
    };
    ...
    // Use a cache key based on jam_id + time range (no bookmark_id)
    ...
    let cancel_flag = state.separation_cancel.clone();
    cancel_flag.store(false, Ordering::Release);
